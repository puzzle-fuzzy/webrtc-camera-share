...
`--receivers` 只接受 `1`、`2`、`4` 或 `8`，`--duration` 范围为 5 秒到 24 小时。脚本会在仅回环可见的临时端口启动服务，用模拟摄像头建立真实 PeerConnection，并每秒采集 `/health`、受保护的 `/metrics` 和浏览器 `getStats()`；`summary.json` 不写入房间、访问码、接收链接或指标令牌。输出目录已有摘要时脚本会拒绝覆盖。
...
页面底部的“关于”可查看项目架构说明，`GitHub` 会打开源码仓库：`https://github.com/puzzle-fuzzy/webrtc-camera-share`。

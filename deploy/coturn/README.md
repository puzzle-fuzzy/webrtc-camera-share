...
`turnserver.conf.example` 只保存非敏感的协议、配额和日志策略。Compose 在启动时通过命令行注入 realm、外部 IP 映射、TLS 文件、端口和 `static-auth-secret`，避免把密钥或环境特定端口提交到仓库。
...
- 安全组与主机防火墙放行 `.env` 中 `TURN_LISTENING_PORT` 的 TCP/UDP、`TURN_TLS_PORT` 的 TCP，以及 `TURN_MIN_PORT`–`TURN_MAX_PORT` 的 UDP；
...
docker compose -f compose.example.yml --env-file .env exec coturn turnutils_stunclient -p "${TURN_LISTENING_PORT:-3478}" 127.0.0.1
```
